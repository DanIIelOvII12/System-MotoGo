...
import { tipoVehiculos } from "../api/tipoVehiculos";
const emptyForm = {
  nombre: "",
  activo: "Activo",
};
...
    setLoadingList(true);
    try {
      const res = await tipoVehiculos.list();
      setItems(res.data);
    } catch (err) {
      setError(
...
      };
      if (isEditing) {
        await tipoVehiculos.update(editingId, data);
      } else {
        await tipoVehiculos.create(data);
      }
      startCreate();
      await loadList();
    } catch (err) {
...
    try {
      const res = await tipoVehiculos.getById(id);
...
            : "Crear nuevo"}
        </h3>
        <form onSubmit={submit} className="crud-form-grid crud-form-grid--vehicles">
          {/* NOMBRE */}
          <div>
            <label>Nombre</label>
            <input
...
{searchResult ? (
  <div className="crud-search-result">
    ...
    <div className="search-result-body">
      ...
      <div className="search-result-item">
        <span>ACTIVO</span>
        <strong>{searchResult.activo}</strong>
      </div>
